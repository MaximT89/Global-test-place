...
class Blur(private val context : Context) {
    ...
    private fun doBlur(sentBitmap: Bitmap, radius: Int, canReuseInBitmap: Boolean): Bitmap? {
        ...
        val bitmap: Bitmap = if (canReuseInBitmap) sentBitmap
        else sentBitmap.copy(sentBitmap.config, true)
    ...
    fun applyBlur(from: View, into: View, scaleFactor : Float = 1f, radius: Float = 20f) {
        from.viewTreeObserver.addOnPreDrawListener(object : ViewTreeObserver.OnPreDrawListener {
